class Question {
  constructor(question, options, answer_index) {
    if (options.length != 4)
      return (
        this ||
        console.error(
          "options parameter item count can't higher or lower than 4!"
        )
      )
...
var QuestionManager = new (class _QuestionManager {
  ...
  init() {
    ...
    document.addEventListener("keydown", event => {
      if (event.key == this.shortcuts.option_0)
        QuestionManager.node_options_0?.click()
      else if (event.key == this.shortcuts.option_1)
        QuestionManager.node_options_1?.click()
      else if (event.key == this.shortcuts.option_2)
        QuestionManager.node_options_2?.click()
      else if (event.key == this.shortcuts.option_3)
        QuestionManager.node_options_3?.click()
      else if (event.key == this.shortcuts.option_pass)
        QuestionManager.node_options_pass?.click()
    })
  ...
  new_question() {
    ...
    this.a += 1
    this.node_question.innerText =
      this.current_question.question + "." + this.a.toString()
  ...
  update_timer = that => {
    that.node_timer.innerText = time_to_string(
      new Date().getTime() - that.start_time
    )
  }

  update_counters() {
    this.node_remaining_questions.innerText =
      this.remaining_questions.toString() +
      "/" +
      this.total_questions.toString()
  }
  ...
  result() {
    ...
    let scoreboard = add_to_scoreboard(this.stats)
    scoreboard.sort((a, b) => {
...
var node_stat_correct
var node_stat_incorrect
var node_stat_pass
var node_stat_time
var node_stat_date

function add_to_scoreboard_random(stats) {
  let scoreboard = []
  for (var i = 0; i < 10; i++) {
    ...
    stats_.time =
      (stats_.incorrect * 10 + 30 + Math.floor(Math.random() * 60)) * 1000
    stats_.date.setTime(
      Date.now() - 1000 * 60 * 60 * Math.floor(Math.random() * 240)
    )
...
function add_to_scoreboard(stats) {
  let scoreboard = get_scoreboard()
  scoreboard.push(stats)
...
function get_scoreboard() {
  let scoreboard_string = Cookies.get("scoreboard") ?? ""
...
function set_scoreboard(scoreboard) {
...
function add_to_scoreboard_node(stats, highlight = false) {
  let el = document.createElement("div")
